...
def create_app() -> Flask:
    ...
    def history(ticker: str):
        ...
        try:
            ...
            return jsonify(
                {"error": "Le parametre 'days' doit etre un entier", "code": 400}
            ), 400
        if not 1 <= days <= 365:
            return jsonify(
                {"error": "Le parametre 'days' doit etre entre 1 et 365", "code": 400}
            ), 400
        try:
    ...
    def sentiment():
        ...
        if not text:
            return jsonify(
                {"error": "Champ 'text' manquant dans le body JSON", "code": 400}
            ), 400
        try:
            result = analyze(text)
    ...
    def sentiment_batch():
        ...
        if not isinstance(texts, list) or not texts:
            return jsonify(
                {"error": "Champ 'texts' (liste non vide) requis", "code": 400}
            ), 400
        if len(texts) > 100:
            return jsonify(
                {"error": "Maximum 100 textes par requete", "code": 400}
            ), 400
        try:
    ...
    def db_stats():
        with SessionLocal() as session:
            ...
            tickers = [t[0] for t in session.query(NewsItem.ticker).distinct().all()]
        return jsonify(
            {
